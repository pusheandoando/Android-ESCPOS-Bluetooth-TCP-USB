// androidescpos/src/main/java/com/volqorstudios/thermalprinter/PrinterCommands.java
...
public class PrinterCommands {
    ...
    public static final int QRCODE_MODEL_1 = 49;
    public static final int QRCODE_MODEL_2 = 50;

    private DeviceConnection deviceConnection;
    private PrinterCharset printerCharset;
    ...
    public static byte[] buildGSv0Header(int bytesByLine, int bitmapHeight) {
    ...
    public static byte[] bitmapToBytes(Bitmap bitmap, boolean gradient) {
        ...
        byte[] imageBytes = PrinterCommands.buildGSv0Header(bytesByLine, bitmapHeight);
            ...
            for (int j = 0; j < bitmapWidth; j += 8) {
                ...
                for (int k = 0; k < 8; k++) {
                    ...
                    if (posX < bitmapWidth) {
                        ...
                        boolean shouldPrintDot = gradient && (red + green + blue) < ((greyscaleCoefficient * gradientStep + greyscaleLine) * colorLevelStep);
                        boolean shouldPrintBW = !gradient && (red < 160 || green < 160 || blue < 160);

                        if (shouldPrintDot || shouldPrintBW) {
                            b |= 1 << (7 - k);
                        }
    ...
    public static byte[][] convertGSv0ToEscAsterisk(byte[] bytes) {
        ...
        returnedBytes[0] = PrinterCommands.LINE_SPACING_24;
        ...
        for (int i = 0; i < imageLineHeightCount; ++i) {
            ...
            imageBytes[imageBytes.length - 1] = PrinterCommands.LF;
            returnedBytes[i + 1] = imageBytes;
        }

        returnedBytes[returnedBytes.length - 1] = PrinterCommands.LINE_SPACING_30;
        return returnedBytes;
    }

    public static byte[] buildQRCodeBytes(String data, int size) throws PrinterBarcodeException {
        ...
        try {
            ...
            e.printStackTrace();
            throw new PrinterBarcodeException("Unable to encode QR code.");
        }

        if (byteMatrix == null) {
            return PrinterCommands.buildGSv0Header(0, 0);
        }
        ...
        int bytesByLine = (int) Math.ceil(((float) imageWidth) / 8f);
        int i = 8;
        ...
        byte[] imageBytes = PrinterCommands.buildGSv0Header(bytesByLine, imageHeight);
    ...
    public PrinterCommands(DeviceConnection deviceConnection) {
        this(deviceConnection, null);
    }

    public PrinterCommands(DeviceConnection deviceConnection, PrinterCharset printerCharset) {
        this.deviceConnection = deviceConnection;
        this.printerCharset = printerCharset != null ? printerCharset : new PrinterCharset("windows-1252", 6);
    }

    public PrinterCommands connect() throws PrinterConnectionException {
    ...
    public PrinterCommands reset() {
        if (!this.deviceConnection.isConnected()) {
            return this;
        }
        this.deviceConnection.write(PrinterCommands.RESET_PRINTER);
        return this;
    }

    public PrinterCommands setAlign(byte[] align) {
        if (!this.deviceConnection.isConnected()) {
            return this;
        }
        this.deviceConnection.write(align);
        return this;
    }

    public PrinterCommands printText(String text) throws PrinterEncodingException {
        return this.printText(text, null, null, null, null, null, null);
    }

    public PrinterCommands printText(String text, byte[] textSize) throws PrinterEncodingException {
        return this.printText(text, textSize, null, null, null, null, null);
    }

    public PrinterCommands printText(String text, byte[] textSize, byte[] textColor) throws PrinterEncodingException {
        return this.printText(text, textSize, textColor, null, null, null, null);
    }

    public PrinterCommands printText(String text, byte[] textSize, byte[] textColor, byte[] textReverseColor) throws PrinterEncodingException {
        return this.printText(text, textSize, textColor, textReverseColor, null, null, null);
    }

    public PrinterCommands printText(String text, byte[] textSize, byte[] textColor, byte[] textReverseColor, byte[] textBold) throws PrinterEncodingException {
        return this.printText(text, textSize, textColor, textReverseColor, textBold, null, null);
    }

    public PrinterCommands printText(String text, byte[] textSize, byte[] textColor, byte[] textReverseColor, byte[] textBold, byte[] textUnderline) throws PrinterEncodingException {
        return this.printText(text, textSize, textColor, textReverseColor, textBold, textUnderline, null);
    }

    public PrinterCommands printText(String text, byte[] textSize, byte[] textColor, byte[] textReverseColor, byte[] textBold, byte[] textUnderline, byte[] textDoubleStrike) throws PrinterEncodingException {
        if (!this.deviceConnection.isConnected()) {
            return this;
        }

        if (textSize == null) {
            textSize = PrinterCommands.TEXT_SIZE_NORMAL;
        }
        if (textColor == null) {
            textColor = PrinterCommands.TEXT_COLOR_BLACK;
        }
        if (textReverseColor == null) {
            textReverseColor = PrinterCommands.TEXT_COLOR_REVERSE_OFF;
        }
        if (textBold == null) {
            textBold = PrinterCommands.TEXT_WEIGHT_NORMAL;
        }
        if (textUnderline == null) {
            textUnderline = PrinterCommands.TEXT_UNDERLINE_OFF;
        }
        if (textDoubleStrike == null) {
            textDoubleStrike = PrinterCommands.TEXT_DOUBLE_STRIKE_OFF;
        }

        try {
            byte[] textBytes = text.getBytes(this.printerCharset.getName());
            this.deviceConnection.write(this.printerCharset.getCommand());

            if (!Arrays.equals(this.currentTextSize, textSize)) {
                this.deviceConnection.write(textSize);
                this.currentTextSize = textSize;
            }

            if (!Arrays.equals(this.currentTextDoubleStrike, textDoubleStrike)) {
                this.deviceConnection.write(textDoubleStrike);
                this.currentTextDoubleStrike = textDoubleStrike;
            }

            if (!Arrays.equals(this.currentTextUnderline, textUnderline)) {
                this.deviceConnection.write(textUnderline);
                this.currentTextUnderline = textUnderline;
            }

            if (!Arrays.equals(this.currentTextBold, textBold)) {
                this.deviceConnection.write(textBold);
                this.currentTextBold = textBold;
            }

            if (!Arrays.equals(this.currentTextColor, textColor)) {
                this.deviceConnection.write(textColor);
                this.currentTextColor = textColor;
            }

            if (!Arrays.equals(this.currentTextReverseColor, textReverseColor)) {
                this.deviceConnection.write(textReverseColor);
                this.currentTextReverseColor = textReverseColor;
            }

            this.deviceConnection.write(textBytes);
    ...
    public PrinterCommands useEscAsteriskCommand(boolean enable) {
        this.useEscAsteriskCommand = enable;
        return this;
    }

    public PrinterCommands printImage(byte[] image) throws PrinterConnectionException {
        if (!this.deviceConnection.isConnected()) {
            return this;
        }

        byte[][] bytesToPrint = this.useEscAsteriskCommand
            ? PrinterCommands.convertGSv0ToEscAsterisk(image)
            : new byte[][]{image};

        for (byte[] bytes : bytesToPrint) {
            this.deviceConnection.write(bytes);
    ...
    public PrinterCommands printBarcode(BarcodeData barcodeData) {
        if (!this.deviceConnection.isConnected()) {
            return this;
        }

        String code = barcodeData.getCode();
        int barcodeLength = barcodeData.getCodeLength();
        byte[] barcodeCommand = new byte[barcodeLength + 4];
        System.arraycopy(
            new byte[]{0x1D, 0x6B, (byte) barcodeData.getBarcodeType(), (byte) barcodeLength},
            0,
            barcodeCommand,
            0,
            4
        ...
        this.deviceConnection.write(new byte[]{0x1D, 0x48, (byte) barcodeData.getTextPosition()});
        this.deviceConnection.write(new byte[]{0x1D, 0x77, (byte) barcodeData.getColWidth()});
        this.deviceConnection.write(new byte[]{0x1D, 0x68, (byte) barcodeData.getHeight()});
        this.deviceConnection.write(barcodeCommand);
        return this;
    }

    public PrinterCommands printQRCode(int qrCodeModel, String text, int size) throws PrinterEncodingException {
        if (!this.deviceConnection.isConnected()) {
            return this;
        }
        ...
        try {
            ...
            this.deviceConnection.write(new byte[]{0x1D, 0x28, 0x6B, 0x04, 0x00, 0x31, 0x41, (byte) qrCodeModel, 0x00});
            this.deviceConnection.write(new byte[]{0x1D, 0x28, 0x6B, 0x03, 0x00, 0x31, 0x43, (byte) size});
            this.deviceConnection.write(new byte[]{0x1D, 0x28, 0x6B, 0x03, 0x00, 0x31, 0x45, 0x30});

            byte[] qrCodeCommand = new byte[textBytes.length + 8];
            System.arraycopy(
                new byte[]{0x1D, 0x28, 0x6B, (byte) pL, (byte) pH, 0x31, 0x50, 0x30},
                0,
                qrCodeCommand,
                0,
                8
            ...
            this.deviceConnection.write(qrCodeCommand);
            this.deviceConnection.write(new byte[]{0x1D, 0x28, 0x6B, 0x03, 0x00, 0x31, 0x51, 0x30});
    ...
    public PrinterCommands newLine() throws PrinterConnectionException {
        return this.newLine(null);
    }

    public PrinterCommands newLine(byte[] align) throws PrinterConnectionException {
        if (!this.deviceConnection.isConnected()) {
            return this;
        }

        this.deviceConnection.write(new byte[]{PrinterCommands.LF});
        this.deviceConnection.send();
    ...
    public PrinterCommands feedPaper(int dots) throws PrinterConnectionException {
        if (!this.deviceConnection.isConnected()) {
            return this;
        }

        if (dots > 0) {
            this.deviceConnection.write(new byte[]{0x1B, 0x4A, (byte) dots});
            this.deviceConnection.send(dots);
        }

        return this;
    }

    public PrinterCommands cutPaper() throws PrinterConnectionException {
        if (!this.deviceConnection.isConnected()) {
            return this;
        }

        this.deviceConnection.write(new byte[]{0x1D, 0x56, 0x01});
        this.deviceConnection.send(100);
        return this;
    }

    public PrinterCommands openCashBox() throws PrinterConnectionException {
        if (!this.deviceConnection.isConnected()) {
            return this;
        }

        this.deviceConnection.write(new byte[]{0x1B, 0x70, 0x00, 0x3C, (byte) 0xFF});
        this.deviceConnection.send(100);
        return this;
    }

    public PrinterCommands printAllCharsetsEncodingCharacters() {
    ...
    public PrinterCommands printCharsetsEncodingCharacters(int[] charsetsId) {
    ...
    public PrinterCommands printCharsetEncodingCharacters(int charsetId) {
        if (!this.deviceConnection.isConnected()) {
            return this;
        }

        try {
            this.deviceConnection.write(new byte[]{0x1B, 0x74, (byte) charsetId});
            this.deviceConnection.write(PrinterCommands.TEXT_SIZE_NORMAL);
            this.deviceConnection.write(PrinterCommands.TEXT_COLOR_BLACK);
            this.deviceConnection.write(PrinterCommands.TEXT_COLOR_REVERSE_OFF);
            this.deviceConnection.write(PrinterCommands.TEXT_WEIGHT_NORMAL);
            this.deviceConnection.write(PrinterCommands.TEXT_UNDERLINE_OFF);
            this.deviceConnection.write(PrinterCommands.TEXT_DOUBLE_STRIKE_OFF);
            this.deviceConnection.write(("Charset " + charsetId + ": ").getBytes());
            this.deviceConnection.write(new byte[]{
            ...
            this.deviceConnection.write(new byte[]{PrinterCommands.LF, PrinterCommands.LF, PrinterCommands.LF, PrinterCommands.LF});
            this.deviceConnection.send();
